...
/* take an object of BusDevice as input of constructor
 * instead of inheritent a child class of BusDeivce (I2CDevice)
 */
ADXL345::ADXL345(BusDevice *bus){
	this->bus = bus;
	...
	this->pitch = 0.0;
	this->roll = 0.0;
	this->bus->writeRegister(POWER_CTL, 0x08);
...
int ADXL345::updateFormatReg(){
	...
	return this->bus->writeRegister(DATA_FORMAT, data_format);
}
...
int ADXL345::readSensorState(){
	this->registers = this->bus->readRegisters(BUFFER_SIZE, 0x00);
	// sanity check
...
void ADXL345::debugSantityCheck(int numOfRd){
	cout << "DEVID = " << "0x" << setfill('0') << setw(2) << hex << int(this->bus->readRegister(0x00)) << endl;
	this->bus->debugDumpRegisters(numOfRd);
}
