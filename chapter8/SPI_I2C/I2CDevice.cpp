...
I2CDevice::I2CDevice(int i2cBusNum, int i2cDeviceAddr):
	BusDevice(i2cBusNum, i2cDeviceAddr){
 ...
 */
int I2CDevice::write(unsigned char address){
...
unsigned char* I2CDevice::readRegisters(unsigned int numOfRd, unsigned int rdAddr){
	...
	if(::read(this->fd, buffer, numOfRd) != (int)numOfRd) {
 ...
 */
void I2CDevice::debugDumpRegisters(unsigned int numOfRd){
